...
- **How many transactions lose a multi-row conflict is not stable.**
  `occ_multirow_predicate` recorded DSQL failing both sides on 2026-09-17 and
  one side on 2026-09-18, from the same probe. The emulator always fails exactly
  one: it adjudicates on the backend's row locks, so whichever transaction takes
  them first is by construction able to commit. The row counts match on both
  sides. The case carries a `KnownGap` rather than being made to pass, because
  the thing it would assert is not something DSQL does the same way twice, and
  reproducing the both-fail outcome means modelling DSQL's adjudication rather
  than borrowing PostgreSQL's, which is the premise the whole layer rests on.
...
(`/* dsql_job=<job id> */ CREATE INDEX ...`), so the id handed back is findable
...
| Job id shape | **Answered.** A job id is 26 characters of lowercase base32: across the ten ids the record holds, all 32 characters of the RFC 4648 alphabet appear and none of `0`, `1`, `8` or `9` ever does, which rules out Crockford's. Unpadded, sixteen bytes come to exactly 26 characters — the same 128 bits a UUID carries, which is consistent with `CALL sys.wait_for_job('no-such-job')` answering `22P02` "Unable to convert text to UUID": the id is decoded rather than compared as text. The emulator issues the same shape, and `sys.wait_for_job` refuses anything that is not of it with DSQL's message. Whether DSQL would also accept a dashed UUID is not observed and does not matter, since neither side issues one. |
...
| Who loses when the conflict spans several rows? | **It varies.** The 2026-09-17 run failed both sides; the 2026-09-18 run failed one. The emulator always leaves a winner, so the case is recorded and replayed but not enforced; see below. |

Answered by the identity probes (recorded 2026-09-18):

| Question | Answer |
|----------|--------|
| Must an identity column be `bigint`? | Yes. `integer` and `smallint` are both refused with `0A000 datatype <type> not supported, identity column type must be bigint`, and `bigint GENERATED BY DEFAULT AS IDENTITY (CACHE 1)` succeeds. |
| Is `ALTER TABLE ADD COLUMN` restricted the same way? | Yes, with the same code and message. |

Open, with probes written and waiting on a run:

- Whether `ALTER TABLE ADD CONSTRAINT ... PRIMARY KEY` and `... UNIQUE` are
  refused. `alter_add_pk_constraint` and `alter_add_unique_constraint` answer it.
  Issue #3 quotes `0A000 unsupported ALTER TABLE ADD CONSTRAINT statement`, and
  a rule was written on that; what the record already pins is the exception, as
  `alter_unique_using_index` shows DSQL accepting the `USING INDEX` form and
  complaining about the index instead.
- What DSQL does with a composite row value, reported in issue #4 as
  `42804 attribute 1 of type "Orders" has wrong type`. `q_row_constructor`,
  `q_row_comparison`, `q_whole_row_reference` and `q_whole_row_in_subquery`
  answer it; all four pass on the emulator today. **No rule was written for
  this.** A rule refusing row values would have to guess which of them DSQL
  actually rejects, and refusing one it accepts is worse than the gap itself —
  the emulator would then fail code that works.
