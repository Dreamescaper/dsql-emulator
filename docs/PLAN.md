...
| Question | Answer |
|----------|--------|
| Is `ALTER TABLE ADD CONSTRAINT ... PRIMARY KEY` refused? | Yes, with `0A000 unsupported ALTER TABLE ADD CONSTRAINT statement`, and `... UNIQUE` the same. The `USING INDEX` form stays accepted, as `alter_unique_using_index` shows. |
| Does DSQL reject composite row values? | **No, in every shape probed.** `SELECT ROW(1, 'a')` returns `[1 a]`, `ROW(1,'a') = ROW(1,'a')` returns `true`, and a whole-row reference — on its own and inside a subquery — returns `(00000000-…-aa,seed)`. |

The second answer is why issue #4 has no rule. Composite values are not refused
as a class, so a rule that refused them would fail code a cluster runs. Whatever
the reported `42804 attribute 1 of type "Orders" has wrong type` comes from, it
is narrower than "a query carries a row value", and pinning it needs a probe
