...
### 1. Record the four identity probes

`identity_integer`, `identity_smallint`, `identity_bigint_by_default` and
`alter_add_identity_integer` are written and answered by observation, but no
recording holds them, so `make conformance` reports four cases as not covered.
The cluster refused a connection on the last attempt, so this wants a fresh
token and a quiet cluster.
