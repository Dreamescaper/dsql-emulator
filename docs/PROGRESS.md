...
`exit=0`, no missing relations, 17 relations verified before and after the
probes, and 226 cases recorded. **The emulator matches all 226**, with only the
accepted `alter_unique_using_index` divergence.

**Issue #2 is confirmed by observation.** `integer` and `smallint` identity
columns are refused with
`0A000 datatype <type> not supported, identity column type must be bigint`,
`bigint GENERATED BY DEFAULT AS IDENTITY (CACHE 1)` succeeds, and
`ALTER TABLE ADD COLUMN` carries the same restriction with the same code and
message. The rules written from the discarded runs' evidence are right, and now
rest on the record rather than on a reading of it.

**The multi-row conflict gap was a race, not a rule.** `occ_multirow_predicate`
recorded *both* transactions failing on 2026-09-17 and *one* on 2026-09-18, from
the same probe. So DSQL does not always reject every side of a conflict spanning
several rows; how many lose is not stable between runs. The earlier entry said
"DSQL failed both transactions" as though it were the behaviour, and that was an
overreading of a single sample — which the entry did flag as one sample, and
which a second sample has now corrected.

The case still carries a `KnownGap`, but for a different reason than before: not
because DSQL fails both and the emulator cannot, but because the number of
survivors is not something DSQL does the same way twice, so there is nothing
stable to enforce. The emulator always leaves exactly one winner, which agrees
with this recording and disagreed with the last.

The two non-conflicting probes also answered correctly this run —
`occ_fk_nonkey_update` and `occ_disjoint_writes` both committed on both sides —
which is the evidence that the previous run's conflicts on those pairs were the
cluster's state and not its behaviour.

Files: `internal/conformance/suite.go`, `test/conformance/golden/backlog.json`,
`test/conformance/golden/occ_conflict.json`, `README.md`, `docs/PLAN.md`.

**Verification.**

```
setup verified: 17 relations
recorded identity_integer             error 0A000
recorded identity_smallint            error 0A000
recorded identity_bigint_by_default   CREATE TABLE
recorded alter_add_identity_integer   error 0A000
recorded occ_multirow_predicate       session 0: BEGIN | UPDATE 3 | COMMIT || session 1: BEGIN | UPDATE 3 | error 40001
Recorded 226 cases against aurora-dsql
Changed 2 fixture(s) in test/conformance/golden: backlog, occ_conflict

$ make conformance
    known gap alter_unique_using_index: ...
    conformance_test.go:114: 226 cases match the golden record

$ make build && make vet && make test && make test-integration
8 packages ok
ok  	github.com/Dreamescaper/dsql-emulator/test/conformance	5.455s
ok  	github.com/Dreamescaper/dsql-emulator/test/integration	8.704s
```

Four runs were needed for one recording. The two guards added along the way —
retrying a schema conflict, and verifying the schema before and after the probes
— are what made the difference between a clean run and a plausible-looking bad
one, and they cost nothing on a run that goes well.

### Refuse a non-bigint identity column, and stop a bad run overwriting the record (2026-09-18)

Reported as [#2](https://github.com/Dreamescaper/dsql-emulator/issues/2): the
emulator accepted `integer GENERATED BY DEFAULT AS IDENTITY`, which Aurora DSQL
refuses with `0A000 datatype integer not supported, identity column type must be
bigint`. An ORM that maps a 32-bit key onto an identity column therefore passed
its whole suite against the emulator and failed on the first `CREATE TABLE`
against a cluster — the exact failure this project exists to prevent, and in the
more dangerous direction, because the emulator was the more permissive of the
two.

A rule now refuses an identity column of any type but `bigint`, on `CREATE
TABLE` and on `ALTER TABLE ADD COLUMN`, naming the type as DSQL names it through
the `{type}` placeholder added earlier. Four probes were written to settle it
against the cluster: the integer and smallint forms, the `bigint` form that the
restriction leaves, and the `ALTER` one.

**The probes are not recorded, and that needs saying plainly.** Three runs were
attempted. Two reached the cluster and both recorded, consistently and early,
`0A000` for integer, smallint and the `ALTER` form, and success for `bigint` —
which is what the rules were written on. Both runs were then discarded, and the
third could not connect at all.

That is the other half of this entry.

**Run one** failed in setup with `40001 schema has been updated by another
transaction (OC001)` on `CREATE TABLE baseline_fk_cascade`. Aurora DSQL
adjudicates schema changes against the catalog, and the suite's setup had grown
to 54 statements. Nothing was written, which is the behaviour a failed run
should have. Setup and cleanup now retry a `40001`, which is what the service
asks a client to do; a probe's own steps are never retried, because what they
answered is the record.

**Run two** wrote a record in which 50 probes reported "relation does not
exist". `baseline_parent` had gone missing partway through, after setup had
created it and the run had used it. Only reading the diff caught it: the save
logic compares content, so a wrong-but-different record overwrites a good one
just as readily as a right one.

**Run three**, from a verified-clean cluster, still lost the schema near the end:
the last eleven probes reported missing relations, and — worse, because it looks
plausible — `occ_fk_nonkey_update` and `occ_disjoint_writes` recorded conflicts
on pairs that cannot conflict. Discarded too.

So `Suite.Verify` now reads one row from every relation the setup creates, before
the probes and again after them. A setup that reports success without leaving the
schema in place, or a run that loses a relation partway through, fails and writes
nothing. `baseline_drop_me` is excluded because a probe removes it on purpose,
and a test keeps the list in step with what setup creates.

Files: `rules/rules.go`, `rules/dsql-2026.09.yaml`,
`internal/classify/eval.go`, `internal/classify/message.go`,
`internal/classify/classify_test.go`, `internal/conformance/record.go`,
`internal/conformance/suite.go`, `internal/conformance/conformance_test.go`,
`README.md`, `docs/PLAN.md`.
...
$ make build && make vet && make test && make test-integration
9 packages ok
ok  	github.com/Dreamescaper/dsql-emulator/test/conformance	5.435s
ok  	github.com/Dreamescaper/dsql-emulator/test/integration	8.691s
```

The emulator refuses the integer and smallint forms with DSQL's message, accepts
`bigint`, and leaves the identity cache refusal answering an identity column
that has no `CACHE` at all, which the record already holds. `make conformance`
reports the four new probes as not covered, which is the signal that the
question is answered by observation rather than by the record.

**Still open.** Record the four identity probes. The evidence is consistent
across two runs but is not in the golden record, and the third run's
`access denied` suggests the cluster or the token needed a rest.
...
| 2026-09-18 | Verify the schema before and after the probes | The save logic compares content, so a wrong record overwrites a good one as readily as a right one. Two of three runs produced records that looked plausible and were not; only a diff caught the first. |
