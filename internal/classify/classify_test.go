...
// is. The recorded probe answers about the constraint rather than the type, so
// the column's type does not enter into it: a bigint identity column, which
// CREATE TABLE accepts, cannot be added to a table that exists either.
func TestClassifyRefusesAddColumnWithAnIdentityConstraint(t *testing.T) {
	c := newClassifier(t)
	...
	for _, sql := range []string{
		...
		"ALTER TABLE t ADD COLUMN seq bigint GENERATED ALWAYS AS IDENTITY (CACHE 65536)",
	} {
		t.Run(sql, func(t *testing.T) {
			result, err := c.Classify(sql)
			if err != nil {
