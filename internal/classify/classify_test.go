...
// ALTER TABLE ADD COLUMN carries the same restriction, which is what an ORM's
// migration generates once a model has grown a new key.
func TestClassifyRefusesNonBigintIdentityOnAlter(t *testing.T) {
	c := newClassifier(t)

	result, err := c.Classify("ALTER TABLE t ADD COLUMN seq integer GENERATED BY DEFAULT AS IDENTITY (CACHE 1)")
	if err != nil {
		t.Fatalf("classify: %v", err)
	}
	if !result.Verdict.Rejected() {
		t.Fatal("an integer identity column was accepted by ALTER TABLE")
	}
	if want := "datatype integer not supported, identity column type must be bigint"; result.Verdict.Message != want {
		t.Fatalf("got %q want %q", result.Verdict.Message, want)
	}

	if result, err := c.Classify("ALTER TABLE t ADD COLUMN seq bigint GENERATED BY DEFAULT AS IDENTITY (CACHE 1)"); err != nil {
		t.Fatalf("classify: %v", err)
