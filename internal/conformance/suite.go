...
func DefaultSuite() Suite {
	cases := []Case{
		...
		{Name: "alter_add_identity_bigint", Group: "backlog", Note: "the type CREATE TABLE accepts, added to a table that exists",
			Steps: one("ALTER TABLE baseline_alter ADD COLUMN seq2 bigint GENERATED BY DEFAULT AS IDENTITY (CACHE 1)")},
		{Name: "alter_add_column_not_null", Group: "backlog", Note: "is NOT NULL a constraint for this purpose?",
			Steps: one("ALTER TABLE baseline_alter ADD COLUMN nn int NOT NULL DEFAULT 0")},
		{Name: "alter_add_column_default", Group: "backlog", Note: "a default on its own",
			Steps: one("ALTER TABLE baseline_alter ADD COLUMN df int DEFAULT 7")},
		{Name: "alter_add_column_check", Group: "backlog", Note: "a check constraint on the new column",
			Steps: one("ALTER TABLE baseline_alter ADD COLUMN ck int CHECK (ck > 0)")},
		{Name: "alter_add_column_unique", Group: "backlog", Note: "a unique constraint on the new column",
