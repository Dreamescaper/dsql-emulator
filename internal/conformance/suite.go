...
func DefaultSuite() Suite {
	cases := []Case{
		...
		{Name: "savepoint", Group: "backlog", Note: "confirm savepoints are refused", Steps: one("SAVEPOINT baseline_sp")},
		{Name: "create_view", Group: "backlog", Note: "confirm a view is allowed, which is why no rule refuses one", Steps: one("CREATE VIEW baseline_v AS SELECT 1 AS x")},
		{Name: "create_sequence", Group: "backlog", Note: "confirm a sequence with no CACHE is refused", Steps: one("CREATE SEQUENCE baseline_seq")},
		...
		{Name: "identity_integer", Group: "backlog", Note: "an integer identity column: refused, naming the type?",
			Steps: one("CREATE TABLE baseline_identity_int (id integer GENERATED BY DEFAULT AS IDENTITY (CACHE 1) PRIMARY KEY)")},
		{Name: "identity_smallint", Group: "backlog", Note: "confirm the restriction is to bigint rather than to integer alone",
			Steps: one("CREATE TABLE baseline_identity_small (id smallint GENERATED BY DEFAULT AS IDENTITY (CACHE 1) PRIMARY KEY)")},
		{Name: "identity_bigint_by_default", Group: "backlog", Note: "the form the restriction leaves, with the smallest allowed CACHE",
		...
		// index instead.
		{Name: "alter_add_pk_constraint", Group: "backlog", Note: "add a primary key to a table that has none",
			Steps: one("ALTER TABLE baseline_alter_pkadd ADD CONSTRAINT baseline_pkadd_pk PRIMARY KEY (id)")},
		{Name: "alter_add_unique_constraint", Group: "backlog", Note: "add a unique constraint without an index to adopt",
		...
		{Name: "q_row_constructor", Group: "backlog", Note: "a row value built in the projection",
			Steps: one("SELECT ROW(1, 'a')")},
		{Name: "q_row_comparison", Group: "backlog", Note: "two row values compared",
			Steps: one("SELECT ROW(1, 'a') = ROW(1, 'a') AS same")},
		{Name: "q_whole_row_reference", Group: "backlog", IgnoreRows: true, Note: "a table referenced as a whole row, which is a composite of its rowtype",
			Steps: one("SELECT baseline_parent FROM baseline_parent LIMIT 1")},
		{Name: "q_whole_row_in_subquery", Group: "backlog", IgnoreRows: true, Note: "the shape an ORM generates for a nested projection",
			Steps: one("SELECT (SELECT p FROM baseline_parent p WHERE p.id = c.id) AS whole FROM baseline_parent c LIMIT 1")},
		{Name: "alter_add_identity_integer", Group: "backlog", Note: "recorded as a refusal about the constraint, not about the type",
