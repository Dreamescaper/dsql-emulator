...
type Suite struct {
	Name  string
	Setup []string
	// Verify runs after Setup and must succeed. Aurora DSQL adjudicates schema
	// changes, so a setup statement can report success and still leave the
	// schema not as it was asked for; the probes would then record a run of
	// "relation does not exist" as though it were the cluster's behavior, and
	// the record would be overwritten with it. One read per shared relation
	// turns that into a failed run, which writes nothing.
	Verify  []string
	Cleanup []string
	Cases   []Case
}
...
// verifyStatements read one row from every relation the setup creates, so a
// setup that reported success but did not leave the schema in place fails the
// run rather than being recorded. TestSetupRelationsAreVerified keeps this in
// step with setupStatements.
func verifyStatements() []string {
	var out []string
	// baseline_drop_me is absent: the drop_table probe removes it on purpose,
	// and these run after the probes as well as before them.
	for _, table := range []string{
		"baseline_parent", "baseline_child", "baseline_idx", "baseline_bulk",
		"baseline_alter", "baseline_alter_pk",
...
func DefaultSuite() Suite {
	cases := []Case{
		...
		// The backlog group holds probes for questions no recording has answered
		// yet, and nothing else: once a run answers one, the case moves to the
		// group it belongs to by subject. TestBacklogHoldsOnlyOpenQuestions
		// keeps it that way, so the group stays a list of what is unknown
		// rather than a pile of everything that was ever asked.
		{Name: "savepoint", Group: "transaction", Note: "confirm savepoints are refused", Steps: one("SAVEPOINT baseline_sp")},
		{Name: "create_view", Group: "supported", Note: "confirm a view is allowed, which is why no rule refuses one", Steps: one("CREATE VIEW baseline_v AS SELECT 1 AS x")},
		{Name: "create_sequence", Group: "supported", Note: "confirm a sequence with no CACHE is refused", Steps: one("CREATE SEQUENCE baseline_seq")},
		{Name: "create_sequence_cached", Group: "supported", Note: "sequences with high CACHE may be allowed", Steps: one("CREATE SEQUENCE baseline_seq2 CACHE 65536")},
		{Name: "create_schema", Group: "supported", Steps: one("CREATE SCHEMA baseline_schema")},
		...
		{Name: "create_function_plpgsql", Group: "unsupported", Note: "confirm non-sql function languages are refused", Steps: one("CREATE FUNCTION baseline_plpgsql() RETURNS int LANGUAGE plpgsql AS $$ BEGIN RETURN 1; END $$")},
		{Name: "identity_column_cached", Group: "supported", Note: "confirm identity with a large CACHE is allowed", Steps: one("CREATE TABLE baseline_identity_cached (id bigint GENERATED ALWAYS AS IDENTITY (CACHE 65536) PRIMARY KEY, value text)")},
		// Reported in issue #2: only a bigint identity column is accepted, so
		// an int key -- what an ORM maps a 32-bit key onto -- fails on a
		// cluster while the emulator used to allow it.
		{Name: "identity_integer", Group: "supported", Note: "an integer identity column: refused, naming the type?",
			Steps: one("CREATE TABLE baseline_identity_int (id integer GENERATED BY DEFAULT AS IDENTITY (CACHE 1) PRIMARY KEY)")},
		{Name: "identity_smallint", Group: "supported", Note: "confirm the restriction is to bigint rather than to integer alone",
			Steps: one("CREATE TABLE baseline_identity_small (id smallint GENERATED BY DEFAULT AS IDENTITY (CACHE 1) PRIMARY KEY)")},
		{Name: "identity_bigint_by_default", Group: "supported", Note: "the form the restriction leaves, with the smallest allowed CACHE",
			Steps: one("CREATE TABLE baseline_identity_big (id bigint GENERATED BY DEFAULT AS IDENTITY (CACHE 1) PRIMARY KEY)")},
		// Reported in issue #3: a key cannot be added to a table that exists,
		// where the USING INDEX form above is accepted and complains about the
		// index instead.
		{Name: "alter_add_pk_constraint", Group: "alters", Note: "add a primary key to a table that has none",
			Steps: one("ALTER TABLE baseline_alter_pkadd ADD CONSTRAINT baseline_pkadd_pk PRIMARY KEY (id)")},
		{Name: "alter_add_unique_constraint", Group: "alters", Note: "add a unique constraint without an index to adopt",
			Steps: one("ALTER TABLE baseline_alter_pkadd ADD CONSTRAINT baseline_pkadd_uq UNIQUE (v)")},
		// Reported in issue #4: a query that carries a composite row value is
		// said to fail on DSQL, which has no user-defined types. Nothing here
		// asserts that yet; these record what it does.
		{Name: "q_row_constructor", Group: "queries", Note: "a row value built in the projection",
			Steps: one("SELECT ROW(1, 'a')")},
		{Name: "q_row_comparison", Group: "queries", Note: "two row values compared",
			Steps: one("SELECT ROW(1, 'a') = ROW(1, 'a') AS same")},
		{Name: "q_whole_row_reference", Group: "queries", IgnoreRows: true, Note: "a table referenced as a whole row, which is a composite of its rowtype",
			Steps: one("SELECT baseline_parent FROM baseline_parent LIMIT 1")},
		{Name: "q_whole_row_in_subquery", Group: "queries", IgnoreRows: true, Note: "the shape an ORM generates for a nested projection",
			Steps: one("SELECT (SELECT p FROM baseline_parent p WHERE p.id = c.id) AS whole FROM baseline_parent c LIMIT 1")},
		{Name: "alter_add_identity_integer", Group: "alters", Note: "recorded as a refusal about the constraint, not about the type",
			Steps: one("ALTER TABLE baseline_alter ADD COLUMN seq integer GENERATED BY DEFAULT AS IDENTITY (CACHE 1)")},
		// The refusal above names the constraint rather than the type, so these
		// ask how wide "with constraint" is. Only the identity form is a rule
		// today; the rest are unanswered and the emulator forwards them.
		{Name: "alter_add_identity_bigint", Group: "alters", Note: "the type CREATE TABLE accepts, added to a table that exists",
			Steps: one("ALTER TABLE baseline_alter ADD COLUMN seq2 bigint GENERATED BY DEFAULT AS IDENTITY (CACHE 1)")},
		{Name: "alter_add_column_not_null", Group: "alters", Note: "is NOT NULL a constraint for this purpose?",
			Steps: one("ALTER TABLE baseline_alter ADD COLUMN nn int NOT NULL DEFAULT 0")},
		{Name: "alter_add_column_default", Group: "alters", Note: "a default on its own",
			Steps: one("ALTER TABLE baseline_alter ADD COLUMN df int DEFAULT 7")},
		{Name: "alter_add_column_check", Group: "alters", Note: "a check constraint on the new column",
			Steps: one("ALTER TABLE baseline_alter ADD COLUMN ck int CHECK (ck > 0)")},
		{Name: "alter_add_column_unique", Group: "alters", Note: "a unique constraint on the new column",
			Steps: one("ALTER TABLE baseline_alter ADD COLUMN uq int UNIQUE")},
		// The two the rule leaves out. An explicit NULL is the default
		// nullability rather than a restriction, and a generated column may be
		// unsupported for its own reasons; neither is recorded.
		{Name: "alter_add_column_null", Group: "backlog", Note: "an explicit NULL, which states the default nullability",
			Steps: one("ALTER TABLE baseline_alter ADD COLUMN nl int NULL")},
		{Name: "alter_add_column_generated", Group: "backlog", Note: "a generated column, which may be refused for its own reasons",
			Steps: one("ALTER TABLE baseline_alter ADD COLUMN gn int GENERATED ALWAYS AS (1) STORED")},
		{Name: "create_sequence_cache_one", Group: "supported", Note: "confirm CACHE 1 is allowed", Steps: one("CREATE SEQUENCE baseline_seq_one CACHE 1")},
		{Name: "rollback_to_savepoint", Group: "transaction", Steps: one("ROLLBACK TO SAVEPOINT baseline_sp")},
		{Name: "set_transaction_repeatable_read", Group: "isolation", Note: "confirm SET TRANSACTION is refused even for the supported level", Steps: one("SET TRANSACTION ISOLATION LEVEL REPEATABLE READ")},
		{Name: "set_default_isolation", Group: "isolation", Note: "confirm SET default_transaction_isolation is refused", Steps: one("SET default_transaction_isolation = 'repeatable read'")},
	}
...
func queryCases() []Case {
	return []Case{
		...
		//
		// Recording it narrowed the shape: the outer reference in the WHERE is
		// not required, because the form with no WHERE at all is refused too.
		// An outer reference in the lateral's target list under a LEFT JOIN is
		// enough.
		//
		...
		// then unteaching it. The two controls sit alongside so that a fix on
		// the cluster shows up as the failing cases passing rather than as an
		// unexplained change.
		...
		{Name: "q_lateral_left_no_outer_where", Group: "queries",
			KnownGap: "DSQL refuses this with 42804; a suspected planner defect, reported and tracked rather than emulated",
			Note:     "the same with no WHERE at all, which is refused too: the outer reference in the target list is enough",
